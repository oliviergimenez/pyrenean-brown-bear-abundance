...
We estimate abundance of the Pyrenean brown bear population with data from France and Spain using robust-design capture-recapture models. We consider data from 2008 to 2024 considering the period May to September. 
...
```{r}
rawdata <- read_csv2("databaseCMR2025.csv", col_names = FALSE)
rawdata
```
...
```{r}
dates <- seq(from = as.Date("2008/5/1"), to = as.Date("2024/9/1"), by = "month") %>% 
...
We now define the structure of the robust design. We have 16 primary occasions (the years, from 2008 to 2024) and 5 secondary occasions (from May to September). From the help file (see ?robust), we read that 'The 0 time intervals represent the secondary sessions in which the population is assumed to be closed. The non-zero values are the time intervals between the primary occasions.'. We therefore write:
```{r}
...
For individuals named `Balou`, `Bercero2013`, `Cachou`, `Gribouille`, `Melloux`, `New20_01`, `New20_12`, `S28Slo3`, `ourson_Caramellita_2020_2` and `ourson_Caramellita_2020_3` we know the date of death, let's use this information and right censor them. First, locate them:
```{r}
dataprocessed %>%
...
GammaPrimeNE <- list(formula=~1, fixed = 0) # gamma' = gamma'' = 0, no emigration
GammaDoublePrimeNE <- list(formula=~1,fixed=0) # gamma' = gamma'' = 0, no emigration
...
We fitted 24 models in total, with 4 detection structures, 2 survival structures and 3 emigration structures. It appears that the models with age-dependent survival and heterogeneous detection are best supported by the data. There is emigration, and it is Markovian. Same results as last year by the way. There is no need to carry out model averaging because the AICc of the top model is lower than the other models. 

Let's inspect the parameter estimates of the best model:
```{r}
modelSaph_mkemig$results$real[1:8,]
```

Survival of cubs is around $81\%$, survival of subadults is $95\%$, and that of adults is $96\%$. Regarding the observation process, we have a mixture of lowly and highly detectable individuals. More precisely, we have a proportion $0.80$ of individuals with detection $44\%$ and a proportion $0.20$ of individuals with detection $89\%$.
...
last[mask] <- c(7, 6, 12, 13, 11, 13, 15, 9, 13, 13)
```
...
model <- function() {
    ...
    for (t in 1:n.years){
      for (j in 1:n.sec[t]){
        pmean[t,j] <- mean(p[1:n.ind,t,j])
        yes[t,j] ~ dbin(pmean[t,j], total[t,j])
      }
    }  

     # state matrices
    for (i in 1:n.ind){
      for (j in first[i]:(n.years - 1)){
        s[i,j,1,1] <- phi[i,j] * gPP
        s[i,j,1,2] <- phi[i,j] * (1 - gPP)
        s[i,j,1,3] <- 1 - phi[i,j]
        s[i,j,2,1] <- phi[i,j] * gP
        s[i,j,2,2] <- phi[i,j] * (1 - gP)
        s[i,j,2,3] <- 1 - phi[i,j]
        s[i,j,3,1] <- 0
        s[i,j,3,2] <- 0
        s[i,j,3,3] <- 1
    ...
    for (i in 1:n.ind){
      for (t in 1:n.years){
        o[i,t,1,1] <- upstar[i,t]
        o[i,t,1,2] <- 1 - upstar[i,t]
        o[i,t,2,1] <- 0
        o[i,t,2,2] <- 1
        o[i,t,3,1] <- 0
        o[i,t,3,2] <- 1
    ...
    for (i in 1:n.ind){
      ...
      for (t in (first[i]+1):last[i]){
        z[i,t] ~ dcat(s[i,t-1,z[i,t-1],1:3])   # state equations
        ch[i,t] ~ dcat(o[i,t,z[i,t],1:2])    # observation equations
      } 
    }
}
```
...
pars <- c('pstar','mu','beta','gamP','gamPP','prop')
n.chains <- 3
n.iter <- 20000
n.burnin <- 5000
```
...
```{r out.width = "100%"}
jagsfit.mcmc <- as.mcmc(res_markov)
library(lattice)
...
```{r}
Nmcmc <- matrix(NA, nrow(res_markov$BUGSoutput$sims.list$pstar), n.primary)
for (i in 1:n.primary){
  Nmcmc[,i] <- caught[i] / res_markov$BUGSoutput$sims.list$pstar[,i]
}
